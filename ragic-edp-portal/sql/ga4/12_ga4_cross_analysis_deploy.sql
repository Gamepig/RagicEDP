-- GA4 x ERP Cross Analysis Deployment Script
-- 目的:
-- 1. 一次重建核心 14 張圖所需的 View
-- 2. 明確標示相依順序，避免 summary / monthly View 先於 base View 被重建
-- 3. 搭配 10_ga4_cross_analysis_validation_checks.sql 做部署後驗證
...
WITH source_base AS (
  SELECT
    ...
    g.cvr,
    SUM(g.sessions) OVER (PARTITION BY g.brand_code, g.event_date) AS brand_day_sessions
  FROM `b25h01-ragic.erp_backup.mat_ga4_source_daily` g
  LEFT JOIN `b25h01-ragic.erp_backup.dim_brand_ga4_mapping` bm
    ON g.brand_code = bm.brand_code
),
sales_base AS (
  SELECT brand_code, date, revenue, orders, aov
...
-- 部署後請接著執行:
-- /Users/gamepig/projects/RagicEDP/ragic-edp-portal/sql/ga4/10_ga4_cross_analysis_validation_checks.sql
-- /Users/gamepig/projects/RagicEDP/ragic-edp-portal/sql/ga4/11_sales_reconciliation_ragic_vs_bq.sql
